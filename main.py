...
@app.get("/forecast")
async def forecast(datasets: str = Query(...), lengths: int = Query(...), index: int = 0, dataloader: Optional[List[int]] = Query(None)):
    ...
    batch_x, batch_y, batch_x_mark, batch_y_mark, seq_trend, seq_seasonal, seq_resid = torch.tensor(data[0]).unsqueeze(dim=0), torch.tensor(data[1]).unsqueeze(dim=0), \
