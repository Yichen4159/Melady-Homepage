...
def vali(model, vali_data, vali_loader, criterion, args, device, itr):
    ...
    with torch.no_grad():
        for i, data in tqdm(enumerate(vali_loader)):
            ...
            if args.model == 'GPT4TS_multi' or args.model == 'NLinear_multi':
                outputs = model(batch_x, itr,  seq_trend, seq_seasonal, seq_resid)
            elif args.model == 'FEDformer' or args.model == 'TimesNet' or args.model == 'LightTS':
...
def test(model, test_data, test_loader, args, device, itr):
    ...
    with torch.no_grad():
        for i, data in tqdm(enumerate(test_loader)):
            ...
            batch_y_mark = batch_y_mark.float().to(device)
            if args.model == 'GPT4TS_multi' or args.model == 'NLinear_multi':
                outputs = model(batch_x[:, -args.seq_len:, :], itr,  seq_trend[:, -args.seq_len:, :], seq_seasonal[:, -args.seq_len:, :], seq_resid[:, -args.seq_len:, :])
            elif args.model == 'FEDformer' or args.model == 'TimesNet' or args.model == 'LightTS':
                dec_inp = torch.zeros_like(batch_y[:, -args.pred_len:, :]).float()
                dec_inp = torch.cat([batch_y[:, :args.label_len, :], dec_inp], dim=1).float().to(device)
                outputs = model(batch_x, batch_x_mark, dec_inp, batch_y_mark)
